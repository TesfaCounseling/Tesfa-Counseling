...
def ensure_appointment_video_room(appointment) -> str | None:
    ...
    ends_at = as_utc(appointment.ends_at)
    exp_ts = int(ends_at.timestamp()) + 3600
    audio_only = (
        getattr(appointment, "session_mode", None) is not None
        and appointment.session_mode.value == "audio_only"
    )
    ...
    try:
        ...
        # Room may already exist from a prior attempt — reuse it.
        if response.status_code in (400, 409):
            existing_url = _fetch_daily_room(api_key, room_name)
            if existing_url:
                appointment.video_room_name = room_name
                appointment.video_room_url = existing_url
                return existing_url
