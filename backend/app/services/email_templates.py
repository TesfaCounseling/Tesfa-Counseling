...
def appointment_booked_email(
    *,
    ...
    if video_room_url:
        plain += f"Video session: {video_room_url}\n"
    ...
    if video_room_url:
        rows += _detail_row(
            "Video",
            f'<a href="{_esc(video_room_url)}" style="color:{GREEN_DARK};">Join video session</a>',
        )
    ...
    if video_room_url:
        body += _cta_button("Join video session", video_room_url)
